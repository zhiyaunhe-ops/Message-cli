...
@app.get("/api/wechat/status", summary="微信数据可用性自检")
def wechat_status():
    wx = _wx()
    try:
        return wx.status()
...
@app.get("/api/wechat/overview", summary="微信全局统计：最近 N 天消息量 / 类型 / 活跃会话")
def wechat_overview(
...
@app.get("/api/wechat/sessions", summary="微信最近会话列表")
...
@app.get("/api/wechat/history", summary="微信单个会话的最近消息")
def wechat_history(
...
@app.get("/api/wechat/stats", summary="微信单个会话统计（类型 / 发言排行 / 24 小时）")
